...
const Card = ({ cityInfo, removeCity }) => {
    ...
    const onDeleteLocalStorage = () => {
        const ids = JSON.parse(localStorage.getItem('cities')).filter(elem => elem !== city.id);
        if (ids.length === 0) {
    ...
    return(
        <div className={s.card}>
            <Link to={url} style={{ textDecoration: 'none', color: 'white'}} className={s.component}>
            ...
            <div className={s.icon_delete}> <button onClick={onDeleteLocalStorage}> <Icon name='delete' /> </button> </div>
        </div>
    )
}
